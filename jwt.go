...
const maxBlacklistCacheSize = 10000 // 超过这个容量就会触发清理过期项的机制，应该在一般情况下还算行
...
func (j *JwtService) CleanupBlacklist() {
	...
	// 先快速扫描收集过期项（持有读锁），再批量删除（持有写锁）
	// 这样减少写锁持有时间，提高并发性能

	// 读锁，不阻塞其他读操作
	j.blacklistMu.RLock()
	expiredItems := make([]string, 0, 100) // 预分配容量
