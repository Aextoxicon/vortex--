...
func addLastMsgCacheColumns(db *sql.DB) error {
	_, err := db.Exec(`
		ALTER TABLE conversation_participants
		ADD COLUMN IF NOT EXISTS last_msg_id BIGINT NOT NULL DEFAULT 0,
		ADD COLUMN IF NOT EXISTS last_msg_from_uid BIGINT NOT NULL DEFAULT 0,
		ADD COLUMN IF NOT EXISTS last_msg_content TEXT NOT NULL DEFAULT '',
		ADD COLUMN IF NOT EXISTS last_msg_ts BIGINT NOT NULL DEFAULT 0,
		ADD COLUMN IF NOT EXISTS last_msg_is_recalled INTEGER NOT NULL DEFAULT 0
	`)
	if err != nil {
		return err
	}
	_, err = db.Exec(`
		CREATE INDEX IF NOT EXISTS idx_conversation_participants_last_msg_ts
		ON conversation_participants (user_id, last_msg_ts DESC)
	`)
	return err
}

func addBioColumn(db *sql.DB) error {
	_, err := db.Exec(`
		ALTER TABLE users
